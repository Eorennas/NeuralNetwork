...
# --- 3. Construção do Modelo (FNN com Extração de Características) ---
# Usamos o MobileNetV2 para extrair características, mas apenas as camadas densas do FNN serão treinadas.
base_model = MobileNetV2(input_shape=img_size + (3,),
                         include_top=False,
                         weights='imagenet')
# Congela as camadas do MobileNetV2
base_model.trainable = False

# Adiciona camadas de uma FNN após a extração de características
x = base_model.output
x = GlobalAveragePooling2D()(x)  # Extraímos as características
x = Dense(512, activation='relu')(x)  # Camada densa FNN
x = Dropout(0.5)(x)  # Regularização com Dropout
x = Dense(128, activation='relu')(x)  # Camada adicional
predictions = Dense(1, activation='sigmoid')(x)  # Saída binária

model = Model(inputs=base_model.input, outputs=predictions)

# Compila o modelo com métricas adicionais
...
summary_text = "\n".join(summary_str)
with open(os.path.join(output_dir, "fnn_model_summary.txt"), "w") as f:
...
checkpoint = ModelCheckpoint(
    filepath=os.path.join(output_dir, "fnn_best_model.keras"),
    monitor='val_loss',
    save_best_only=True,
    verbose=1
)
...
with open(os.path.join(output_dir, "fnn_training_time.txt"), "w") as f:
    f.write(f"Tempo total de treinamento: {training_time:.2f} segundos")


# --- 6. Salvar Modelo Final ---
model.save('fnn_final_model.keras')
print(f"\nModelo final salvo em 'fnn_final_model.keras'")
